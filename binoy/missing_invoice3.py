    ...
    def invoice_creation(self, args):
		...
		try:
			...
			if month_from_date in date_list:
			    ...
			    monthfrom = monthfrom.replace(day=invoice_date)
                            continue
			if month_from_date < today_date:
			    ...
			    create_invoice_from= create_invoice_from.replace(day=invoice_date)
                            end = create_invoice_from.replace(hour=23, minute=59, second=59, microsecond=0)
			    end = end  + relativedelta(days=-1)
                            local_end = create_invoice_from.replace(hour=23, minute=59, second=59, microsecond=0)
			    local_end = local_end + relativedelta(days=-1)
