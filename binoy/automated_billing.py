...
class BillingEngine():
	    ...
	    print("---------------------------")
            ...
            if invoice:
                    ...
                    for items in invoice:
                        ...
                        invoice_date_list[tenant_id].append(invoice_date)
                        invoice_date_for_tenant = invoice_date_list.get(tenant_id, None)
			logging.info('*************invoice_date_for_tenant**************************')
			logging.info(invoice_date_for_tenant)
		    ...
		    print current_month, ' !=',  invoice_month_latest
	  	    logging.info('today %s > invoice_day %s' % (today, invoice_day))
		    logging.info('current_month %s  invoice_month_latest %s' % (current_month, invoice_month_latest))
    ...
    def calc_and_create(self, tenant_id, begin, end, local_begin, local_end, date):
	    ...
	    logging.info("tenant_id= %s tenant_id, begin= %s, end= %s, service='compute', instance_id= %s" % (tenant_id, begin, end, a))
            compute_value_for_instance = ck.reports.get_total(tenant_id=tenant_id, begin=begin, end=end, service='compute', instance_id=a)
	    logging.info("+++++++++++++++++++++++++++++++++++++++++++++++++")	
	    logging.info(compute_value_for_instance)
	    logging.info("+++++++++++++++++++++++++++++++++++++++++++++++++")
