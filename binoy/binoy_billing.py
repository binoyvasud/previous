...
class BillingEngine():
    ...
    def date_check(self, creation_date, tenant_id, date):
	    ...
	    #print 'invoice', invoice
            ...
            if invoice:
                    ...
                    today = date.day
		    logging.info("today %s invoice_day_latest %s" %(today, invoice_day_latest))
                    if today > invoice_day_latest:
                        
                            # list for date range
                            tmpDate = []
			    logging.info("self.spanning_months")
                            for d in self.spanning_months(invoice_date_latest, date):
				logging.info("month %s year %s" % (d.month, d.year))
            ...
            # first invoice
   		    ...
   		    logging.info("Creating the first invoice")
    ...
    def spanning_months(self, start, end):
  	logging.info("in the spanning_months") 
	...
	logging.info('current %s end %s' % (current, end))
        while current <= end:
